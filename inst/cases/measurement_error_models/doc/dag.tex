...
\begin{tikzpicture}[
	...
	thick, % line style
	U/.style={circle, draw=black, inner sep=1.8pt, outer sep=1.5pt, minimum size=3mm, font=\small},  %draw=black, fill=white
	O/.style={circle, inner sep=1.4pt, outer sep=-.75pt, minimum size=3mm, font=\small},              %draw=white, fill=white
	]
    ...
    \node[O] (Z) [below right =.85 of X] {$Z$};
    \node[O] (Y) [below left = .85 of X] {$Y$};
       
	% Paths connecting nodes
    \path[->] (X) edge node[above left,scale=.5]{$~~\beta_{yx}$} (Y);
    \path[->] (X) edge (Xo);
    \path[->] (Z) edge node[above right,scale=.5]{$\beta_{xz}$} (X);
    \path[->] (Z) edge node[scale=.5]{$\beta_{yz}$} (Y);
\end{tikzpicture}
