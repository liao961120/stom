...
\begin{tikzpicture}[
    ...
    \node[O] (R1) [above left = .65 of E1] {$Q$};
   	\node[O] (DD1) [below left = .65 of D1] {$D^\ast$};
   	\node[O] (T) [right = .85 of D1] {$T$};
       
	% Paths connecting nodes
    \path[->] (T) edge node[outer sep=2pt, scale=.75]{$~~~\pmb{\beta_{TD}}$} (D1);
    \path[->] (T) edge[densely dashed] node[above right,scale=.75]{$\pmb{\beta_{TE}}$} (E1);
    \path[->] (E1) edge (R1);
    \path[->] (E1) edge[densely dashed] node[scale=.75]{$\pmb{\beta_{ED}}$} (D1);
    \path[->] (A)  edge node[below,scale=.75]{$\pmb{\beta_{AD}}~$} (D1);
    \path[->] (A) edge node[scale=.75]{$\pmb{\beta_{AE}}$} (E1);
    \path[->] (D1) edge (DD1);
\end{tikzpicture}
