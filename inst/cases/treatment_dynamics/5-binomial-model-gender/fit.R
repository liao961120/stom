...
set.seed(100)
d = sim_data( outcome="binom", alpha=-.5 )
...
predict_obs = function(Sid, Tid=NULL, A=NULL, G=NULL, time=1:4, idx=1) {
...
plot_model_prediction = function(Sid=85) {
    ...
    pred = sapply( sample(1:3000, 30), function(i) {
        D = predict_obs(Sid=Sid, idx=i)
        14 * logistic(-D)  # expected counts
    })
    # Compute posterior means with all samples
    pred2 = sapply( 1:3000, function(i) {
        D = predict_obs(Sid=Sid, idx=i)
        14 * logistic(-D)  # expected counts
    })
