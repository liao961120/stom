...
fit = m$sample(data=sim,
               ...
               seed = 1234,
               save_warmup = TRUE,
               init = write_init_files(sim, seed=40)
)
...
post[post$rhat > 1.02,]
# Trace plot
param_inspec = post[post$rhat > 1.02,]$variable
color_scheme_set("viridis")
mcmc_trace(fit$draws(), pars = param_inspec, facet_args = list(ncol=3))
############################
