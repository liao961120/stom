/*
    Binomial Outcome model, Full model
*/
...
parameters {
    ...
    // Subject efficacy (varying intercepts + slopes)
    matrix[2,Ns] Z_subj;
...
transformed parameters {
    ...
    for ( i in 1:NO ) {
        int sid = Sid_O[i];
        int time = time_O[i];
        real muE = delta + E_subj[sid,1] + E_subj[sid,2]*time + B_AE*A[i] + B_TE[G[i],Tx[i]]*time;
...
model {
    ...
    // Causes of E (transformed parameters block)
